...
#df['BMI'] = 100**2*df['weight']/(df['height']*df['height'])
df['overweight'] = 100**2*df['weight']/(df['height']*df['height'])>25
...
def draw_cat_plot():
    ...
    df_cat = pd.melt(df, var_name = 'variable', value_vars = ['active','alco','cholesterol', 'gluc','overweight','smoke'],
                 id_vars = 'cardio')

    # Draw the catplot with 'sns.catplot()'
    fig = sns.catplot(data=df_cat, kind="count",  x="variable",hue="value",
